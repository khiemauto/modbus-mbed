/*
    ModbusSerial.h - Header for ModbusSerial Library
    Copyright (C) 2019 Khiem Tran
*/
#include <mbed.h>
#include <Modbus.h>

#ifndef MODBUSSERIAL_H
#define MODBUSSERIAL_H


#define MODBUS_BUFFER_SIZE	256

typedef struct {
    uint16_t bits;
    SerialBase::Parity parity;
    uint16_t stop_bits;
} SerialFormat;

class ModbusSerial : public Modbus {
    
    private:
        Serial* _port;
        long  _baud;
        u_int _format;
        DigitalOut  *_txPin;
        unsigned int _t15; // inter character time out
        unsigned int _t35; // frame delay
        uint8_t  _slaveId;

		Timeout _request;
		uint8_t _rxbuffer[MODBUS_BUFFER_SIZE];
		uint16_t _rxlen;

		void interruptRx();
		void responeRequest();
        uint16_t calcCrc(uint8_t address, uint8_t* pduframe, uint8_t pdulen);
    public:
        ModbusSerial();

		/** Set slave ID for modbus, range of ID is 1-255
     	*
     	*  @param slaveId Slave ID
     	*/
		bool setSlaveId(uint8_t slaveId);			//Set
        uint8_t getSlaveId();

		/** Config for modbus protocol
     	*
     	*  @param port Pointer to serial
     	*  @param baud Baudrate for modbus
     	*  @param format SerialFormat with bits, parity, stopbit.
     	*  @param txPin Pin control direction of module TTL to RS485. eg: PA_5, PB_3...
     	*/
        bool config(Serial* port, long baud, SerialFormat format, int txPin=-1);
        bool receive(uint8_t* frame);
        void sendPDU(uint8_t* pduframe);
        void send(uint8_t* frame);
};

/* Table of CRC values for high�order byte */
const uint8_t _auchCRCHi[] = {
    };

/* Table of CRC values for low�order byte */
const uint8_t _auchCRCLo[] = {
    };

#endif //MODBUSSERIAL_H
